...
class TreasureHuntServer
{
    ...
    private static string ProcessCommand(string clientId, string command)
    {
        lock (lockObj)
        {
            ...
            double oldDistance = Math.Sqrt(Math.Pow(oldPosition.X - treasure.X, 2) + Math.Pow(oldPosition.Y - treasure.Y, 2));
            double newDistance = Math.Sqrt(Math.Pow(newPosition.X - treasure.X, 2) + Math.Pow(newPosition.Y - treasure.Y, 2));

            playerPositions[clientId] = newPosition;
            UpdateMap();

            string feedback = newDistance < oldDistance ? "You are getting closer to the treasure!" : "You are getting farther from the treasure!";
            if (newPosition == treasure)
            {
                playerScores[clientId]++;
                AssignNewTreasure();
                BroadcastScores();
                feedback = "You found the treasure! " + feedback;
            }

            return $"Current location: ({newPosition.X}, {newPosition.Y}). {feedback}";
        }
    }

    private static void AssignNewTreasure()
    {
        lock (lockObj)
        {
            int newX, newY;
            do
            {
                newX = random.Next(0, 10);
                newY = random.Next(0, 10);
            } while (map[newY, newX] != '.');
    ...
    private static void BroadcastScores()
    {
        lock (lockObj)
        {
            string scoresMessage = "Current scores:\n";
            foreach (var player in playerScores)
            {
                scoresMessage += $"Player {playerNumbers[player.Key]}: {player.Value} points\n";
            }

            byte[] messageBytes = Encoding.UTF8.GetBytes(scoresMessage);
    ...
    private static void UpdateMap()
    {
        ...
        foreach (var player in playerPositions)
        {
            ...
            map[pos.Y, pos.X] = positionCounts[pos] > 1 ? '*' : playerNum.ToString()[0]; // Display player number or '*' if overlapping
        }
    }
    ...
    private static void HandleClient(TcpClient client)
    {
        ...
        lock (lockObj)
        {
            ...
            playerScores[clientId] = 0; // Initialize score

            UpdateMap();
            DisplayMap(); // Display map immediately when player connects
        }
